...
GOAL_DIR=$(find .goals -maxdepth 1 -type d -name "${GOAL_ID}-*" | head -1)
...
for TASK_FILE in "${OPEN_TASKS[@]}"; do
  ...
  # Mark in-progress before handing off to pingo
  sed -i '' 's/^status: open$/status: in-progress/' "$TASK_FILE"

  PINGO_EXIT=0
  OUTPUT=$(pingo pipeline --task "$TASK_FILE" 2>&1) || PINGO_EXIT=$?

  echo "$OUTPUT"

  if [[ $PINGO_EXIT -ne 0 ]]; then
    ...
    echo ""
    echo "HALT: ${TASK_ID} failed (exit ${PINGO_EXIT})"
